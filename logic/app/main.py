...
@app.get("/")
def read_root():
    return {"message": "Application Tracker API v2.0", "status": "running", "database": "SQLite"}
...
@app.post("/applications/{application_id}/job-details", response_model=JobDetailResponse, status_code=status.HTTP_201_CREATED)
def create_job_details(application_id: int, job_detail: JobDetailCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(JobDetail).filter(JobDetail.application_id == application_id).first()
    if existing:
        raise HTTPException(status_code=400, detail="Job details already exist for this application")
...
def update_job_details(application_id: int, job_detail: JobDetailUpdate, db: Session = Depends(get_db)):
    """Update job details for an application"""
    db_job_detail = db.query(JobDetail).filter(JobDetail.application_id == application_id).first()
...
def get_notes(application_id: int, db: Session = Depends(get_db)):
    ...
    notes = db.query(Note).filter(Note.application_id == application_id).order_by(Note.created_at.desc()).all()
    return notes

@app.post("/applications/{application_id}/notes", response_model=NoteResponse, status_code=status.HTTP_201_CREATED)
...
@app.get("/applications/{application_id}/activities", response_model=List[ActivityLogResponse])
def get_activities(application_id: int, db: Session = Depends(get_db)):
    """Get activity log for an application"""
    activities = db.query(ActivityLog).filter(
        ActivityLog.application_id == application_id
    ).order_by(ActivityLog.created_at.desc()).all()
    return activities

# ============== Deadline Endpoints ==============

@app.get("/applications/{application_id}/deadlines", response_model=List[DeadlineResponse])
...
@app.post("/applications/{application_id}/deadlines", response_model=DeadlineResponse, status_code=status.HTTP_201_CREATED)
def create_deadline(application_id: int, deadline: DeadlineCreate, db: Session = Depends(get_db)):
...
def update_deadline(deadline_id: int, deadline: DeadlineUpdate, db: Session = Depends(get_db)):
    ...
    if new_completed != old_completed:
        activity = ActivityLog(
            ...
            activity_type='deadline_completed' if new_completed else 'deadline_reopened',
...
def export_to_excel(db: Session = Depends(get_db)):
    ...
    headers = [
        ...
        'Status', 'Date Applied', 'Deadline', 'Job URL', 'Latest Note'
    ]
    ws.append(headers)
